      ...
      <footer className="border-t border-primary/10 bg-white">
        <div className="mx-auto max-w-7xl px-6 py-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <div>
              <p className="font-serif text-xl text-primary font-bold">{siteName}</p>
            ...
            <div>
              ...
              <ul className="space-y-2.5">
                {footerNav.map((item) => (
                  <li key={item.href}>
                    <Link href={item.href} className="text-sm text-charcoal/50 hover:text-primary transition-colors">{item.name}</Link>
                  </li>
                ))}
              </ul>
            </div>
              ...
              {settings?.reseauxSociaux && settings.reseauxSociaux.length > 0 && (
                <div className="mt-6 flex gap-3">
                  {settings.reseauxSociaux.map((s) => (
                    <a key={s._key} href={s.url} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center text-primary text-xs font-medium hover:bg-primary/20 transition-colors">
